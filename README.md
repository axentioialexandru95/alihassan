...
2. Set the default environment to `https://localhost:3000/`.
   If you run the Next.js dev server on a different port, use that exact HTTPS URL instead, for example `https://localhost:3002/`.
3. Save.
...
```sh
npm run dev:https -- -p 3002
```

The first run generates a self-signed certificate in `certificates/`. Open the HTTPS URL in a separate tab once and accept the certificate warning before loading the preview inside Storyblok.
